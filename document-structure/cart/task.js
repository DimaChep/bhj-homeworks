...
addProducts.forEach((i) => {
  i.addEventListener("click", () => {
    ...
    const imgSrc = [...i.closest(".product").getElementsByTagName("img")][0]
      .src;
    const productQuantityValueAdd = i
      .closest(".product")
      .querySelector(".product__quantity-value").textContent;

    const cart = [...cartProducts.querySelectorAll(".cart__product")];

    const validId = cart.findIndex(
      (elem) => elem.getAttribute("data-id") === dataId
    );
    ...
    if (cart.length === 0 || validId === -1) {
      ...
      let productQuantityValue = cart[validId].querySelector(
        ".cart__product-count"
      ).textContent;
      cart[validId].querySelector(".cart__product-count").textContent =
        Number(productQuantityValue) + Number(productQuantityValueAdd);
    }
